...
def generate_sequences(data: np.ndarray, len_window: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    x_sequences = np.empty(shape=(no_sequences, len_window * 2), dtype=np.float64)
    y_sequences = np.empty(shape=(no_sequences, len_window * 2), dtype=np.float64)

    for i in range(no_sequences):
        x_sequences[i] = get_frequency_domains(data[i * len_window:(i + 1) * len_window])
        y_sequences[i] = get_frequency_domains(data[(i + 1) * len_window:(i + 2) * len_window])
